...
const Navbar = () => {
  ...
  const toggleDropdown = (index) => setOpenDropdowns((prev) => (prev.includes(index) ? prev.filter((i) => i !== index) : [...prev, index]));
      ...
      <div className="strax-responsive-nav">
        <div className="container">
          <div className="strax-responsive-menu">
            ...
            <button type="button" className={styles.mobileToggle} aria-label="Toggle navigation" onClick={() => setMobileOpen((v) => !v)}>
              {mobileOpen ? (
                <svg className={styles.meanClose} width="16" height="16" viewBox="0 0 16 16" aria-hidden="true">
                  <path d="M1 1 L15 15 M15 1 L1 15" stroke="#ee4132" strokeWidth="2" strokeLinecap="round" fill="none" />
                </svg>
              ) : (
                <>
                  <span></span>
      ...
      <div className={`strax-nav ${mobileOpen ? "menu-open" : ""}`}>
        <div className="container">
          <nav className="navbar navbar-expand-md navbar-light">
            ...
            <div className={`collapse navbar-collapse mean-menu ${mobileOpen ? styles.show : ""}`} id="navbarSupportedContent">
              <ul className="navbar-nav">
                ...
                <li className={itemClass(["/about-us", "/eeiith"])}>
                  <span className="nav-link" onClick={() => toggleDropdown(0)}>
                    About
                  </span>
                  <button type="button" className="mean-expand-btn" aria-label="Expand About menu" onClick={() => toggleDropdown(0)}>
                ...
                <li className={itemClass(["/nice", "/tihan", "/idex-dio", "/nidhi-prayas", "/meity-tide", "/aisea", "/build", "/sisfs"])}>
                  <span className="nav-link" onClick={() => toggleDropdown(1)}>
                    Programs
                  </span>
                  <button type="button" className="mean-expand-btn" aria-label="Expand Programs menu" onClick={() => toggleDropdown(1)}>
                    {openDropdowns.includes(1) ? "−" : "+"}
                  </button>
                  <ul className={`dropdown-menu ${openDropdowns.includes(1) ? "dropdown-open" : ""}`}>
                    <li className="nav-item">
                      <Link to="/nice" className={linkClass(["/nice"])}>
                        NICE
                      </Link>
                    </li>
                    <li className="nav-item">
                      <Link to="/tihan" className={linkClass(["/tihan"])}>
                        TiHAN
                      </Link>
                    </li>
                    <li className="nav-item">
                      <Link to="/idex-dio" className={linkClass(["/idex-dio"])}>
                        iDEX
                      </Link>
                    </li>
                    <li className="nav-item">
                      <Link to="/nidhi-prayas" className={linkClass(["/nidhi-prayas"])}>
                        NIDHI PRAYAS
                      </Link>
                    </li>
                    <li className="nav-item">
                      <Link to="/meity-tide" className={linkClass(["/meity-tide"])}>
                        Meity TIDE 2.0
                      </Link>
                    </li>
                    <li className="nav-item">
                      <Link to="/aisea" className={linkClass(["/aisea"])}>
                        AISEA
                      </Link>
                    </li>
                    <li className="nav-item">
                    ...
                    <li className="nav-item">
                      <Link to="/sisfs" className={linkClass(["/sisfs"])}>
                        SISFS
                      </Link>
                    </li>
                  </ul>
                ...
                <li className={itemClass(["/board", "/mentors", "/team", "/careers"])}>
                  <span className="nav-link" onClick={() => toggleDropdown(2)}>
                    People
                  </span>
                  <button type="button" className="mean-expand-btn" aria-label="Expand People menu" onClick={() => toggleDropdown(2)}>
                ...
                <li className={itemClass(["/startups", "/startup"])}>
                  <Link to="/startups" className={linkClass(["/startups", "/startup"])}>
                    Startups
                  </Link>
                </li>
                <li className={itemClass(["/reports", "/downloads"])}>
                  <span className="nav-link" onClick={() => toggleDropdown(3)}>
                    Media
                  </span>
                  <button type="button" className="mean-expand-btn" aria-label="Expand Media menu" onClick={() => toggleDropdown(3)}>
              ...
              <div className="others-options">
                <Link to="https://docs.google.com/forms/d/e/1FAIpQLSevtsrrvYA-noBRMfmRvtKZu51kA2gEgXbtWvLDHlcVDwQ0Gg/viewform" className="default-btn" style={{ paddingLeft: "5px", paddingRight: "5px" }} onClick={closeMobile} target="_blank" rel="noopener noreferrer">
